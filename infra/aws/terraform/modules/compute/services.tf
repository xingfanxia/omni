...
# Nextcloud Connector Service
resource "aws_ecs_service" "nextcloud_connector" {
  count = contains(var.enabled_connectors, "nextcloud") ? 1 : 0

  name            = "omni-${var.customer_name}-nextcloud-connector"
  cluster         = var.cluster_arn
  task_definition = aws_ecs_task_definition.nextcloud_connector[0].arn
  launch_type     = "FARGATE"
  desired_count   = var.desired_count
  ...
  service_registries {
    registry_arn = aws_service_discovery_service.nextcloud_connector[0].arn
  }
